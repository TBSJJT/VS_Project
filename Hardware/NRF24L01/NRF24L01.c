...
void NRF24L01_Init(void)
{
	...
	W_Reg(W_REGISTER+CONFIG,0x0F);//配置成接收模式 00001111
	W_Reg(W_REGISTER+EN_AA,0x01);//通道0开启自动应答
	W_Reg(W_REGISTER+RF_CH,0x40);//配置通信频率2.4G
	W_Reg(W_REGISTER+RX_PW_P0,32);//配置接收通道0接收的数据宽度32字节
	W_Reg(W_REGISTER+EN_RXADDR,0x01);//接收通道0使能
	W_Reg(W_REGISTER+SETUP_RETR,0x1A);//配置500us重发时间间隔,重发10次 0001：间隔 1010：重发次数
	W_Reg(FLUSH_RX,NOP);
